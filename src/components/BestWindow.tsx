...
export default function BestWindow({ hourly, date }: Props) {
  ...
  return (
    <div className="bestwin">
      <WeatherIcon kind={info.icon} isDay={result.isDay} size={26} />
      <div className="bestwin-txt">
        <span className="bestwin-lead">Nejlepší okno na ven</span>
        <strong>
          {result.startH}–{result.endH} h
        </strong>
        <span className="bestwin-sub">{info.label.toLowerCase()}</span>
      </div>
    </div>
  );
}
